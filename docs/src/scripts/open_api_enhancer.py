...
def enhance(properties):
    ...
    with open(Path(__file__).parent.parent.parent / 'modules/ROOT/assets/attachments/takshashila.yml', 'r') as yaml_file:
...
def update_schema_description(schema_detail):
    if (schema_detail.get('x-field-extra-annotation')):
        ...
        if match:
            ...
            if possible_values:
                if (not schema_detail.get('example') and schema_detail.get('type') and not schema_detail['type'] == 'array'):
                ...
                schema_detail['description'] = f"{schema_detail.get('description', '')}. Possible values: {', '.join(possible_values)}"
...
def fetch_picklist_values(picklist_name):
    ...
    with mariadb.connect(user=db_username, password=db_password,
        ...
        with connection.cursor() as cursor:
            values = []
            for row in cursor.execute("SELECT NAME FROM PICKLIST WHERE PICKLIST_NAME=:pn AND LOCALE='en' ORDER BY PICKLIST_ORDER ASC", pn=picklist_name):
                if row[0]:
                    values.append(row[0])
            return values
